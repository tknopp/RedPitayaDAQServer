...
file delete -force $build_dir/tmp/$project_name.cache $build_dir/tmp/$project_name.hw $build_dir/tmp/$project_name.srcs $build_dir/tmp/$project_name.runs $build_dir/tmp/$project_name.xpr

create_project -part $part_name $project_name $build_dir/tmp

set_property IP_REPO_PATHS $build_dir/tmp/cores [current_project]

update_ip_catalog

set bd_path $build_dir/tmp/$project_name.srcs/sources_1/bd/system

create_bd_design system

source linux-image/cfg/ports.tcl
...
source linux-image/projects/$project_name/block_design.tcl
...
set files [glob -nocomplain linux-image/projects/$project_name/*.v linux-image/projects/$project_name/*.sv]
if {[llength $files] > 0} {
  add_files -norecurse $files
}

set files [glob -nocomplain linux-image/cfg/*.xdc linux-image/projects/$project_name/*.xdc]
if {[llength $files] > 0} {
