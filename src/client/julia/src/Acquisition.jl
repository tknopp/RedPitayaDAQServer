...
See [`readSamples`](@ref)
"""
function startPipelinedData(
  rpu::Union{RedPitayaCluster, RedPitayaClusterView},
  reqWP::Int64,
  numSamples::Int64,
  chunkSize::Int64,
)
...
"""
Checks for the current writepointer if a timeout occured.
"""
...
function correctFilterDelay(rpu::Union{RedPitaya, RedPitayaCluster}, wpStart)
  return correctFilterDelay(wpStart, decimation(rpu))
end
...
function readSamples(
  ...
  wpStart::Int64,
  numOfRequestedSamples::Int64;
  chunkSize::Int64 = 25000,
  rpInfo = nothing,
  correct_filter_delay::Bool = true,
)
...
function collectSamples!(
  ...
  return put!(channel, SampleChunk(result, performances))
end
...
function readFrames(
  ...
  rawSamples = readSamples(
    rpu,
    Int64(wpStart),
    Int64(numOfRequestedSamples);
    chunkSize = chunkSize,
    rpInfo = rpInfo,
    correct_filter_delay = correct_filter_delay,
  )
