export RedPitayaClusterView,
  master, numChan, readFrames, readPeriods, startPipelinedData, collectSamples!, viewToCluster, clusterToView
...
function RedPitayaClusterView(rpc::RedPitayaCluster, selection::Vector{Bool})
  length(selection) == length(rpc) ||
    throw(DimensionMismatch("Length of boolean vector must match length of cluster"))
...
currentWP(rpcv::RedPitayaClusterView) = currentWP(rpcv.rpc)
